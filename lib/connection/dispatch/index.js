...
class Dispatch {
  ...
  _resolveHook(name, version, opts, cb) {
    ...
    if (version) {
      if (typeof version !== 'number' && typeof version !== 'string') {
        ...
        console.warn(new Error(`[dispatch] hook: using implied latest version for "${name}"`));
      }
    }
    ...
    if (name === '*') {
      ...
      const normalizedName = (name.indexOf('_') === -1)
        ? name.replace(/[A-Z]/g, '_$&').toUpperCase()
        : name;

      code = protocol.map.name.get(normalizedName);
      if (code == null) {
        const origText = (normalizedName !== name) ? ` (original: "${name}")` : '';
        console.warn(new Error(`unrecognized hook target "${normalizedName}"${origText}`));
        code = '_UNKNOWN';
      }
    }
  ...
  unhook() {
    // TODO
    console.error('Dispatch#unhook() is not implemented yet');
  }
  ...
  write(outgoing, name, version, data) {
    ...
    if (Buffer.isBuffer(name)) {
      ...
      if (!data && typeof version === 'object') {
        data = version;
        version = '*';
        console.warn(new Error(`[dispatch] write: using implied latest version for "${name}"`));
      }

      try {
        data = protocol.write(name, version, data);
      } catch (e) {
        errMsg(`[dispatch] write: failed to generate ${name}<${version}>`, e, data);
        return false;
      }

      data = this.handle(data.getUint16LE(2), data, !outgoing, true);
  ...
  handle(code, data, fromServer, fake = false) {
