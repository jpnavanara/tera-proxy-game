const protocol = require('tera-data-parser').protocol;
...
class Dispatch {
  ...
  _resolveHook(name, type, cb) {
    ...
    if (name === '*') {
      ...
      const transformedName = (name.indexOf('_') === -1)
        ? name.replace(/[A-Z]/g, '_$&').toUpperCase()
        : name;

      code = protocol.map.name.get(transformedName);
      if (code == null) {
        const origInfo = (transformedName !== name) ? ` (original: "${name}")` : '';
        console.warn(new Error(`unrecognized hook target "${transformedName}"${origInfo}`));
        code = '_unknown';
      }
    }
