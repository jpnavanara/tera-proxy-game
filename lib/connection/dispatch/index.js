...
function getMessageName(protocolVersion, identifier, version, originalName) {
  ...
  if (typeof identifier === 'number') {
    const name = protocol.maps.get(protocolVersion).code.get(identifier) || `(opcode ${identifier})`;
    return `${name}<${version}>`;
  }
...
class Dispatch {
  constructor(connection, protocolVersion = 0) {
    ...
    this.protocolMap = protocol.maps.get(protocolVersion);
    ...
    if (code === 19900 && !this.protocolVersion) { // C_CHECK_VERSION
      ...
      if (parsed instanceof Error) {
        ...
        if (!item || item.index !== 0) {
          ...
          this.protocolVersion = item.value;
          ...
          if (!this.protocolMap) {
            ...
            for (const queued of this.queuedHooks) {
