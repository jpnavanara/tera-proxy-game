...
function logError(message) {
  log.error(Array.isArray(message) ? message.join('\n') : message);
}
...
class Dispatch {
  ...
  load(name, from = module, ...args) {
    ...
    try {
      ...
      // this is important to see, so log it direct to console if log level is too low
      if (Array.isArray(log.streams)) {
        if (log.streams.some(s => s.name === 'console' && s.stream.level > 30)) {
          console.log(`[dispatch] loaded "${name}"`); // eslint-disable-line no-console
        }
      }
